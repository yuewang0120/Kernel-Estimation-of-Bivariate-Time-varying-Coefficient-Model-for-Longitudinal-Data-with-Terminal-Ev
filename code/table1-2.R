load("code/simu_result_beta3=0.5.RData")
source("code/functions.R")
grid <- expand.grid(seq(0, 18, 1), seq(0, 18, 1)) %>% filter(Var1 + Var2 >= 5, Var1 + Var2 <= 18)
truebeta <- c(mapply(beta1, grid[, 1], grid[, 2]), mapply(beta2, grid[, 1], grid[, 2]), rep(0.5, nrow(grid)))
...
for (i in 1:nrow(temp)) {
    temp[i, 4:9] <- test_point(temp$t1[i], temp$T[i] - temp$t1[i], temp$t2[i], temp$T[i] - temp$t2[i])
}
names(temp)[4:9] <- c("delta_beta1", "ep1", "delta_beta2", "ep2", "delta_beta3", "ep3")
temp %>% mutate(across(where(is.numeric), round, digits=3)) %>% print()
sink()
sink("code/table2.txt")
temp <- data.frame(T1 = c(8, 8, 12, 8, 8, 12, 8, 8, 12), T2 = c(12, 16, 16, 12, 16, 16, 12, 16, 16), t = c(2, 2, 2, 4, 4, 4, 6, 6, 6))
for (i in 1:nrow(temp)) {
    temp[i, 4:9] <- test_point(temp$t[i], temp$T1[i] - temp$t[i], temp$t[i], temp$T2[i] - temp$t[i])
}
names(temp)[4:9] <- c("delta_beta1", "ep1", "delta_beta2", "ep2", "delta_beta3", "ep3")
temp %>% mutate(across(where(is.numeric), round, digits=3)) %>% print()
sink()
